...
def generate_sim_matrix(amplicon_dict: dict[set]) -> tuple[list[list[int]], list[str]]:
    ...
    sim_matrix = [[1000 for _ in range(len(amplicon_dict))] for _ in range(len(amplicon_dict))]
...
def finding_clusters(sim_matrix: list[list[float]], org_list: list[str]):
    ...
    while len(np_sim_matrix) > 1:
        # print(np_sim_matrix)
        max_ind_pair = np.argwhere(np_sim_matrix == np_sim_matrix.min())[0]
        ...
        org1_avg = max_value / 2
        org2_avg = max_value / 2
        # print(max_ind_pair)
    ...
    # reduced_sim_matrix = np.zeros((len(np_sim_matrix)-1, len(np_sim_matrix)-1))
        ...
        np_modified_sim = np.insert(np_modified_sim, max_ind_pair[0], 1000.0)
        ...
        if org_list[max_ind_pair[0]] in org_dist:
            org1_avg = (max_value / 2) - org_dist[org_list[max_ind_pair[0]]]
        if org_list[max_ind_pair[1]] in org_dist:
            org2_avg = (max_value / 2) - org_dist[org_list[max_ind_pair[1]]]
        ...
        org_list[max_ind_pair[0]] = '(' + org_list[max_ind_pair[0]] + ":" + str(org1_avg) + "," + org_list[max_ind_pair[1]] + ":" + str(org2_avg) + ')'
        org_dist[org_list[max_ind_pair[0]]] = max_value / 2
