...
def call_ispcr() -> dict[set]:
    ...
    #retrieve the path of the assembly files and perform isPCR on each assembly
...
def generate_sim_matrix(amplicon_dict: dict[set]) -> list[list[int]]:
    ...
    #TD: calculate sim score for one half of diagonal matrix
    # org_pairs = combinations(amplicon_dict.keys(), 2)
    # print(org_pairs)
    # print(amplicon_dict)
    ...
    for ind1, org1 in enumerate(amplicon_dict.keys()):
        for ind2, org2 in enumerate(amplicon_dict.keys()):
            if org1 == org2:
                ...
                # print(next(iter(amplicon_dict[org1])))
                aln, score = upgma.needleman_wunsch(next(iter(amplicon_dict[org1])), next(iter(amplicon_dict[org2])), match, mismatch, gap)
                # sim_matrix[ind1][ind2] = score
                sim_matrix[ind1][ind2] = round(score / max(len(next(iter(amplicon_dict[org1]))), len(next(iter(amplicon_dict[org1])))), 2)
                # break
    for i in sim_matrix:
        print(i)
    return sim_matrix

def normalize_score(sim_matrix: list[list[float]], amplicon_dict: dict[set]) ->list[list[float]]:
...
def finding_clusters(sim_matrix: list[list[float]]): #sim_matrix: list[list[float]]):
    ...
    while len(np_sim_matrix) > 1:
        ...
        print(max_value)
        # for the row anc col containing max value, calculate the average for every other row and col
...
if __name__ == "__main__":
    ...
    finding_clusters(norm_sim_matrix) #norm_sim_matrix)
